...
The **3:10** silent screen recording is available at [`demo/final_demo_video.mp4`](demo/final_demo_video.mp4). It records the actual command `python -m msba_job_matcher.final_demo --recording`, including live initialization of the 100,000-row dataset, a normal search request, grounded retrieval output, an explicit `NO OPT/CPT` edge case, the batch review workflow, and measured evaluation. English headings and explanations remain on screen long enough for the recording to be understood without audio.
...
- Recorded demo: `demo/final_demo_video.mp4`, a silent screen recording of the reproducible real-run command.

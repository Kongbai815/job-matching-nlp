...
The narrated **3:40** demo is available at [`demo/final_demo_video.mp4`](demo/final_demo_video.mp4). It shows a real search request, grounded retrieval output, an explicit `NO OPT/CPT` edge case, a missing-metadata failure case, measured evaluation, and the batch review workflow. The video includes synchronized English narration; [`demo/final_demo_captions.srt`](demo/final_demo_captions.srt) and [`demo/final_demo_narration_script.txt`](demo/final_demo_narration_script.txt) provide accessible text copies.

The demonstrated outputs are generated by the CLI commands in the Usage section and are committed under `outputs/`. The edge case intentionally preserves a `high_fit` role prediction while forcing `review_required=true` and a Hold action, showing that role fit never overrides explicit authorization evidence.
...
- Recorded demo: `demo/final_demo_video.mp4`, with synchronized narration, captions, and transcript.
